	...
	it("Should let you buy a ticket",function(done){
		Conference.new({from:owner_account}).then(
		   function(conference){
		   	 ...
		   	 conference.buyTicket({from:sender_account,value:ticketPrice}).then(
		   	   function(){
		  ...
		  function(conference){
		  	...
		  	conference.buyTicket({from:sender_account,value:ticketPrice}).then(
		  	  function(){
		  	    ...
		  	    var difference = 	newBalance - initialBalance;
		  	    assert.equal(difference, ticketPrice, "Difference should be what was sent");
		  	    return conference.refundTicket(sender_account,ticketPrice,{from:sender_account});
		  	  }).then(
		  	    function(){	
		  	    	var balance = web3.eth.getBalance(conference.address);
		  	    	assert.equal(balance, ticketPrice, "Balance should be unchanged");
		  	    	return conference.refundTicket(sender_account,ticketPrice,{from:owner_account});
		  	  }).then(
		  	    function(){
